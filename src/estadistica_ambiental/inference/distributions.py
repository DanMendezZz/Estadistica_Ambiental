...
def normality_tests(series: pd.Series, alpha: float = 0.05) -> pd.DataFrame:
    ...
    if len(s) <= 5000:
        ...
        results.append({"test": "Shapiro-Wilk", "statistic": round(stat, 4),
                        "pval": round(p, 6), "normal": p >= alpha})

    # Kolmogorov-Smirnov contra normal estándarizada
    s_norm = (s - s.mean()) / s.std() if s.std() > 0 else s
    stat, p = spstats.kstest(s_norm, "norm")
    results.append({"test": "Kolmogorov-Smirnov", "statistic": round(stat, 4),
                    "pval": round(p, 6), "normal": p >= alpha})
    ...
    results.append({"test": "Anderson-Darling", "statistic": round(res.statistic, 4),
                    "pval": None, "normal": res.statistic < crit})
    ...
    for dist_name in dists:
        try:
            ...
            rows.append({
                ...
                "params":       str({p: round(v, 4) for p, v in zip(dist.shapes.split(",") if dist.shapes else [], params)}),
