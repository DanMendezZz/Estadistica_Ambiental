...
NB = pathlib.Path("notebooks/lineas_tematicas/bloque_a_gestion/oferta_hidrica.ipynb")
nb = json.loads(NB.read_text(encoding="utf-8"))
cells = nb["cells"]

# -- Seccion 1b: Piezometria + transmisividad + curva de abatimiento ----------
piezo_md = md(
...
# -- Seccion 3c: Visualizacion piezometrica + curva abatimiento ---------------
piezo_viz_md = md(
...
cells.insert(idx_data + 1, piezo_md)
cells.insert(idx_data + 2, piezo_code)

idx_seasonal = next(i for i, c in enumerate(cells)
                    if c["cell_type"] == "code"
                    and "plot_seasonal_means(" in "".join(c["source"])
                    and "import" not in "".join(c["source"]))
cells.insert(idx_seasonal + 1, piezo_viz_md)
cells.insert(idx_seasonal + 2, piezo_viz_code)
