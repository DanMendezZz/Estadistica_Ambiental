...
NB = pathlib.Path("notebooks/lineas_tematicas/bloque_a_gestion/paramos.ipynb")
nb = json.loads(NB.read_text(encoding="utf-8"))
cells = nb["cells"]

# ── Sección 1b: IRH + caudal + cobertura ──────────────────────────────────────
irh_md = md(
...
# ── Sección 5c: Gradiente Caldas-Lang ────────────────────────────────────────
caldas_md = md(
...
# ── Insertar celdas ────────────────────────────────────────────────────────────
# 1) Seccion 1b: despues de datos sinteticos
idx_data = next(i for i, c in enumerate(cells)
                if c["cell_type"] == "code" and "sintéticos de ejemplo" in "".join(c["source"]))
cells.insert(idx_data + 1, irh_md)
cells.insert(idx_data + 2, irh_code)

# 2) Seccion 3c: despues de plot_seasonal_means (llamada directa, no import)
idx_seasonal = next(i for i, c in enumerate(cells)
                    if c["cell_type"] == "code"
                    and "plot_seasonal_means(" in "".join(c["source"])
                    and "import" not in "".join(c["source"]))
cells.insert(idx_seasonal + 1, irh_viz_md)
cells.insert(idx_seasonal + 2, irh_viz_code)

# 3) Seccion 5c: despues de mann_kendall
idx_mk = next(i for i, c in enumerate(cells)
              if c["cell_type"] == "code" and "mann_kendall(" in "".join(c["source"]))
cells.insert(idx_mk + 1, caldas_md)
cells.insert(idx_mk + 2, caldas_code)
