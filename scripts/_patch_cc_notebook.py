...
NB = pathlib.Path("notebooks/lineas_tematicas/bloque_b_transversales/cambio_climatico.ipynb")
nb = json.loads(NB.read_text(encoding="utf-8"))
cells = nb["cells"]

# ── Sección 1b: Inventario GEI por sector ─────────────────────────────────────
gei_md = md(
...
# ── Sección 3c: NDC progress + stacked ────────────────────────────────────────
ndc_md = md(
...
# ── Sección 7b: Monte Carlo EF Tier 1 vs Tier 2 ───────────────────────────────
mc_md = md(
...
for i, c in enumerate(cells):
    src = "".join(c["source"])
    if c["cell_type"] == "code" and "walk_forward" in src and "gap=" not in src:
        cells[i]["source"] = src.replace(
            "walk_forward(model, ts, horizon=",
            "walk_forward(model, ts, gap=12, horizon="
        )

# ── Insertar celdas en posiciones correctas ────────────────────────────────────
# Buscar por contenido específico (no en imports) usando múltiples condiciones

# 1) Seccion 1b: despues de datos sintéticos de temperatura
idx_data = next(i for i, c in enumerate(cells)
                if c["cell_type"] == "code" and "sintéticos de ejemplo" in "".join(c["source"]))
cells.insert(idx_data + 1, gei_md)
cells.insert(idx_data + 2, gei_code)

# 2) Seccion 3c: despues de plot_seasonal_means (la celda que LLAMA a la función, no la que la importa)
idx_seasonal = next(i for i, c in enumerate(cells)
                    if c["cell_type"] == "code"
                    and "plot_seasonal_means(" in "".join(c["source"])
                    and "import" not in "".join(c["source"]))
cells.insert(idx_seasonal + 1, ndc_md)
cells.insert(idx_seasonal + 2, ndc_code)

# 3) Monte Carlo: despues de rank_models (la celda que LLAMA a rank_models, no la de imports)
idx_rank = next(i for i, c in enumerate(cells)
                if c["cell_type"] == "code"
                and "rank_models(results)" in "".join(c["source"]))
cells.insert(idx_rank + 1, mc_md)
cells.insert(idx_rank + 2, mc_code)
